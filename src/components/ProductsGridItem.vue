<template>
  <v-card width="25vw">
    <v-img :src="imgUrl" contain height="300px">
      <v-container fill-height fluid pa-2>
        <v-layout fill-height>
          <v-flex>
            <span
              class="headline pa-1 grey--text text--lighten-3 elevation-5"
    ...
    <v-card-title style="position: relative;">
      ...
      <div>
        <span class="grey--text text--darken-1"
          >Category: {{ product.category.name.toLowerCase() }}</span
        ><br />
        <v-divider light></v-divider>
        <span v-if="displayMode === 'shop'"
          >Price: <strong>{{ product.price.toFixed(2) }} zł</strong></span
        >
        <span v-else
          >Price: <strong>{{ this.priceOfAllItems.toFixed(2) }} zł</strong> ({{
        ...
        <span v-if="displayMode === 'shop'"
          >Available:
          <strong :class="`${availabilityColor}--text`">{{
            available
          }}</strong></span
        >
        <span v-else
          >Quantity: <strong>{{ this.product.quantity }}</strong></span
        ><br />
      </div>
    </v-card-title>
    <v-card-text>
